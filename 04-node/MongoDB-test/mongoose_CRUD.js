...
// mongoose.set('useCreateIndex',true) //使用一个新的索引创建器 已弃用
...
mongoose.connection.on('open',function (err) {
  if(err){
    ...
    //1 获取配置字段的构造函数（引入模式对象（Schema 计划，模型,刚要）其实就是对文档中的每一个字段的进行配置。）
    //let Schema = mongoose.Schema
    //2.创建配置构造函数的实例对象（创建约束对象）
    let studentsRule = new mongoose.Schema({
      stu_id:{
