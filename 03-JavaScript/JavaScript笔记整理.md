...
`分离读写`: 浏览器对书写代码的优化，将操作DOM元素的代码读写操作整合后再执行（有点像var 声明的变量）
...
![image-20210618152841634](imgs/image-20210618152841634-1624001325400.png)
...
![image-20210618152905369](imgs/image-20210618152905369.png)
...
(function () {
    ...
    init.prototype = jQuery.fn;
  	//把init当做一个类，让这个类的原型指向了jQuery.prototype

    window.jQuery = window.$ = jQuery;
})();
$().filter() //=>创建一个JQUERY类的实例，可以调取JQ.FN中的方法
$.isFunction() //=>把JQ当做一个普通对象，直接的使用对象上扩展的那些私有属性和方法
```
...
+ jQuery(callback)
  ...
  > $(document).[ready()](https://jquery.cuishifeng.cn/ready.html)的简写。

+ each(callback)

  + $.each(target,callback) ——-遍历数组或者类数组

  + $([selector]).each(callback)

  + 内置each，（基于$([selector])获取的集合，默认使用一次 each方法）

    ```javascript
    //给Odiv下的所有span设置样式
    $('#Odiv>span').css('color','red');
    ```
