<template>
  <div class="row">
    ...
    <h2 v-else-if="errorMsg">搜索失败，失败的原因是：{{ errorMsg }}</h2>
...
export default {
  ...
  computed:{
    ...mapState(['users','firstView','loadingView','errorMsg'])
  },
  mounted() {
    //vuex 版本
    ...
    */
    ...
    this.$eventBus.$on("search", async (searchValue) => {
      ...
      try {
        this.loadingView = false;
        //第三屏-请求ajax成功的数据显示
        const result = await axios.get("https://api.githubs.com/search/users", {
          params: {
            q: searchValue,
          },
        });
