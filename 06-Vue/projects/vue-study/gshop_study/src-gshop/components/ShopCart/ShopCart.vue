<template>
  <div>
    <div class="shopcart">
      <div class="content">
        <div class="content-left">
          <div class="logo-wrapper" @click="toggleCartShow">
            <div class="logo" :class="{ highlight: totalCount }">
              <i
                class="iconfont icon-gouwuche"
                :class="{ highlight: totalCount }"
              ></i>
            </div>
            <div class="num" v-show="totalCount">{{ totalCount }}</div>
          </div>
          <div class="price highlight">￥{{ totalPrice }}</div>
          <div class="desc" v-if="info">另需配送费￥{{ info.deliveryPrice }}元</div>
        </div>
        <div class="content-right">
          <div class="pay" :class="payClass">
            {{ payContent }}
          </div>
        </div>
      </div>

      <!-- <div class="shopcart-list" style="display: none"> -->
      <transition name="shopcart-list">
        <div class="shopcart-list" v-show="isCartShow">
          <div class="list-header">
            <h1 class="title">购物车</h1>
            <span class="empty" @click="emptyCartItems">清空</span>
          </div>
          <div class="list-content">
            <ul>
              <li class="food" v-for="(food,index) in cartItems" :key="index">
                <span class="name">{{food.name}}</span>
                <div class="price"><span>￥{{food.price}}</span></div>
                <div class="cartcontrol-wrapper">
                  <div class="cartcontrol">
                    <div class="iconfont icon-remove_circle_outline"></div>
                     <CartControl :food="food"></CartControl>
                    <!-- <div class="cart-count">                   
                    </div> -->
                    <div class="iconfont icon-add_circle"></div>
                  </div>
                </div>
              </li>
            </ul>
          </div>
        </div>
      </transition>
    </div>
    <div class="list-mask" @click="isCartShow = false" v-show="isCartShow"></div>
  </div>
</template>

<script>
import CartControl from '../CartControl/CartControl.vue';
import { mapGetters, mapState } from "vuex";
export default {
  data() {
    return {
      initData: "shopCart初始化数据",
      isCartShow: false,
    };
  },
  components:{
    CartControl
  },
  computed: {
    //mapGetters 只能用数组
    ...mapGetters(["totalCount", "totalPrice"]),

    ...mapState({
      info: (state) => {
        return state.shop.shopData.info;
      },
      cartItems:state=>state.shop.cartItems
    }),

    payContent() {
      let { totalPrice, info } = this;
      //这里需要使用info的值，需要对它进行预处理
      if (!info) return;
      // 还差￥{{ info.minPrice }}元起送
      let text;
      if (totalPrice === 0) {
        text = `${info.minPrice}元起送`;
      } else if (totalPrice < info.minPrice) {
        text = `还差￥${info.minPrice - totalPrice}元起送`;
      } else {
        text = "去结算";
      }

      return text;
    },

    payClass() {
      let { totalPrice, info } = this;
      if (!info) return;
      //高亮显示的样式类 not-enough
      return totalPrice - info.minPrice >= 0 ? "enough" : "not-enough";
    },
  },
  methods: {
    toggleCartShow() {
      //切换显示隐藏 购物车列表
      this.isCartShow = !this.isCartShow;
    },
    emptyCartItems(){
      // this.cartItems=[];//ne setter
      // 通知 vuex做情况操作
      this.$store.dispatch('emptyCartItemsAction');
    }
  },
  watch:{
    cartItems(newValue){
      //购物车中没有商品的时候，自动关闭购物车列表的元素     
      !newValue.length && (this.isCartShow = false);
    }
  }
};
</script>

<style lang="stylus" rel="stylesheet/stylus" scoped>
@import '../../common/stylus/mixins.styl'
.shopcart
  position: fixed
  left: 0
  bottom: 0
  z-index: 50
  // width: 100%
  width 375px
  height: 48px
  .content
    display: flex
    background: #141d27
    font-size: 0
    color: rgba(255, 255, 255, 0.4)
    .content-left
      flex 1
      display flex
      width 275px
      .logo-wrapper
        flex-shrink : 1        
        display: inline-block
        vertical-align: top
        position: relative
        top: -10px
        margin: 0 12px
        padding: 6px
        width: 56px
        height: 56px
        box-sizing: border-box
        border-radius: 50%
        background: #141d27
        .logo
          width: 100%
          height: 100%
          border-radius: 50%
          text-align: center
          background: #2b343c
          &.highlight
            background: $color
          .icon-gouwuche
            line-height: 44px
            font-size: 24px
            color: #80858a
            &.highlight
              color: #fff
        .num
          position: absolute
          top: 0
          right: 0
          width: 24px
          height: 16px
          line-height: 16px
          text-align: center
          border-radius: 16px
          font-size: 9px
          font-weight: 700
          color: #ffffff
          background: rgb(240, 20, 20)
          box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.4)
      .price
        // flex-grow: 3
        flex-shrink: 4
        display: inline-block
        vertical-align: top
        margin-top: 12px
        line-height: 24px
        padding-right: 6px
        box-sizing: border-box
        border-right: 1px solid rgba(255, 255, 255, 0.1)
        font-size: 14px
        font-weight: 700
        &.highlight
          color: #fff
      .desc
        // flex-grow: 6
        flex-shrink 2
        display: inline-block
        vertical-align: top        
        margin: 12px 0 0 2px
        line-height: 24px
        font-size: 10px
        white-space:nowrap;
    .content-right
      flex: 0 0 100px
      flex-shrink:0;
      width: 100px
      .pay
        height: 48px
        line-height: 48px
        text-align: center
        font-size: 12px
        font-weight: 700
        &.not-enough
          background: #2b333b
        &.enough
          background: #00b43c
          color: #fff
  .shopcart-list
    position: absolute
    left: 0
    top: 0
    // top: -100px
    transform: translateY(-100%)
    z-index: -1
    width: 100%
    &.shopcart-list-enter-active, &.shopcart-list-leave-active
      transition all .5s
    &.shopcart-list-enter, &.shopcart-list-leave-to
      transform: translateY(0)
    .list-header
      height: 40px
      line-height: 40px
      padding: 0 18px
      background: #f3f5f7
      border-bottom: 1px solid rgba(7, 17, 27, 0.1)
      .title
        float: left
        font-size: 14px
        color: rgb(7, 17, 27)
      .empty
        float: right
        font-size: 12px
        color: rgb(0, 160, 220)
    .list-content
      padding: 0 18px
      max-height: 217px
      overflow: hidden
      background: #fff
      .food
        position: relative
        padding: 12px 0
        box-sizing: border-box
        bottom-border-1px(rgba(7, 17, 27, 0.1))
        .name
          line-height: 24px
          font-size: 14px
          color: rgb(7, 17, 27)
        .price
          position: absolute
          right: 90px
          bottom: 12px
          line-height: 24px
          font-size: 14px
          font-weight: 700
          color: rgb(240, 20, 20)
        .cartcontrol-wrapper
          position: absolute
          right: 0
          bottom: 6px
.list-mask
  position: fixed
  top: 0
  left: 0
  width: 100%
  height: 100%
  z-index: 40
  backdrop-filter: blur(10px)
  opacity: 1
  background: rgba(7, 17, 27, 0.6)
  &.fade-enter-active, &.fade-leave-active
    transition: all 0.5s
  &.fade-enter, &.fade-leave-to
    opacity: 0
</style>
